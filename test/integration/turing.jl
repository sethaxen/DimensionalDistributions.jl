...
@testset "Turing integration" begin
    ...
    @testset for dim_dist_type in
                 (DimensionalDistributions.AsDimArrayDistribution, TestDimDistribution)
        @model function noncentered_eight(σ; dim=only(dims(σ)), n=length(σ))
            μ ~ Normal(0, 5)
            τ ~ truncated(Cauchy(0, 5); lower=0)
            if dim_dist_type == DimensionalDistributions.AsDimArrayDistribution
                θ_tilde ~ withdims(filldist(Normal(), n), dim)
            else
                θ_tilde ~ TestDimDistribution(Normal(), (dim,))
            end
            θ := @. μ + τ * θ_tilde
