...
function Base.rand(rng::Random.AbstractRNG, dist::AsDimArrayDistribution, lengths::Dims)
    dims = Dimensions.dims(dist)
    xs = map(Base.Fix2(DimensionalData.DimArray, dims), rand(rng, parent(dist), lengths))
    return xs
end
