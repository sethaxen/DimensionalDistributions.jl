...
@static if isdefined(Bijectors, :VectorBijectors)
    ...
    Bijectors.with_logabsdet_jacobian(f::UnwrapDimArray, x::AbstractArray) = (f(x), _zero(x))
    ...
    function VectorBijectors.from_vec(d::DimensionalDistributions.AbstractDimArrayDistribution)
        return WrapDimArray(Dimensions.dims(d)) ∘ Reshape(size(d))
    end
    function VectorBijectors.to_vec(d::DimensionalDistributions.AbstractDimArrayDistribution)
        return Vec(size(d)) ∘ UnwrapDimArray(Dimensions.dims(d))
    end
    function VectorBijectors.from_linked_vec(d::DimensionalDistributions.AbstractDimArrayDistribution)
    ...
    function VectorBijectors.to_linked_vec(d::DimensionalDistributions.AbstractDimArrayDistribution)
        b = Bijectors.bijector(d)
        sz = Bijectors.output_size(b, size(d))
        return Vec(sz) ∘ b ∘ UnwrapDimArray(Dimensions.dims(d))
    end
    function VectorBijectors.linked_vec_length(d::DimensionalDistributions.AbstractDimArrayDistribution)
    ...
    function VectorBijectors.from_linked_vec(d::DimensionalDistributions.AsDimArrayDistribution)
        return WrapDimArray(Dimensions.dims(d)) ∘ VectorBijectors.from_linked_vec(parent(d))
    end
    function VectorBijectors.to_linked_vec(d::DimensionalDistributions.AsDimArrayDistribution)
        return VectorBijectors.to_linked_vec(parent(d)) ∘ UnwrapDimArray(Dimensions.dims(d))
    end
    function VectorBijectors.linked_vec_length(d::DimensionalDistributions.AsDimArrayDistribution)
